...
> projExp []     = TupE []
> projExp (x:xs) = TupE [x, (Just $ projExp xs)]

> projFun p = LamE (map replaceWild p) (projExp (map (Just . VarE) (concatMap patToVarPs p)))
...
>                                     TupP ((map VarP envVars) ++ [TupP []])]] (projExp (map (Just . VarE) (lVars ++ envVars)))
